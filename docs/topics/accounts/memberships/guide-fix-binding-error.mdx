---
title: Fix a user binding 
---

# Fix a user binding

Fix user binding by updating memberships with the API.

## Who can fix binding {#who-can-fix}

Users with binding errors **can resolve some errors themselves:**

- `emailVerifiedMatchError`: User needs to verify their email address through the [authorization URL](../index.mdx#invite).
- `idVerifiedMatchError`: User needs to complete the appropriate [identification process](../../users/identifications/index.mdx).

**For all other binding error types**, only account members with the `canManageAccountMembership` permission and status `Enabled` can resolve the error using the following API mutations:
